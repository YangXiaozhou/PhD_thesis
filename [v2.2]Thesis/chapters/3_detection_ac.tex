...
The one-to-one correspondence between the Jacobian and grid topology can be established by looking at how the admittance matrix is constructed in (\ref{ch2:eqn:admittance_matrix}). $\boldsymbol{Y}_{bus}$ is constructed from the bus incidence matrix ${A}$ and line admittances. For different outage scenarios, the corresponding column of $ A$ need to be set to 0. For example, to set up the $l_{th}$ line outage, the entries in the $l_{th}$ column of ${A}$ are set to 0 to get ${A}_\ell$. The corresponding bus admittance matrix $\boldsymbol{Y}_{bus, \ell}$ is obtained by 
$$
